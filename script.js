...
/* ==========================================
   ...
   (সম্পূর্ণ localStorage রিমুভ — শুধু Firebase)
   ========================================== */
...
// ---- Config (admin password + next order ID) ----
...
async function removeFirebaseUserProfile(uid) {
  try {
    await db.collection('userProfiles').doc(uid).delete();
  } catch (e) { console.error('removeProfile error:', e); }
}
...
/* ==========================================
   WISHLIST (per-device — localStorage এ রাখা হয়)
   ========================================== */
function loadWishlist() {
  try {
    var s = localStorage.getItem('fg_wishlist');
    if (s) wishlist = new Set(JSON.parse(s));
...
function doLogin() {
  ...
  auth.signInWithEmailAndPassword(email, password)
    .then(function (cred) {
      closeLogin();
      getFirebaseUserProfile(cred.user.uid).then(function (profile) {
        var name = profile ? profile.name : (cred.user.displayName || 'User');
        showToast('Welcome back, ' + name + '!');
      });
    })
    .catch(function (error) {
      errEl.textContent = getFirebaseErrorMsg(error.code);
...
function closeLogin() {
  ...
  document.querySelectorAll('.login-modal .co-submit').forEach(function (b) { b.disabled = false; b.textContent = b.id ? 'Send Reset Link' : (b.closest('#regFormDiv') ? 'Create Account' : 'Login'); });
}
...
async function adminDoLogin() {
  ...
  if (u !== ADMIN_DEFAULT_USER || p !== config.adminPass) { err.textContent = 'Invalid username or password'; err.classList.add('show'); return; }
...
async function updateOrderStatus(id) {
  ...
  try {
    await updateOrderInDB(id, { status: newStatus });
    closeAdmOrderModal();
    ...
    try {
      ...
      if (data.products && Array.isArray(data.products)) {
        ...
        for (var i = 0; i < data.products.length; i += batchSize) {
          var batch = db.batch();
          var end = Math.min(i + batchSize, data.products.length);
          for (var j = i; j < end; j++) {
            batch.set(db.collection('products').doc(String(data.products[j].id)), data.products[j]);
          }
          await batch.commit();
        }
        products = await loadProducts();
        await renderAdminProducts();
        showToast('Imported ' + products.length + ' products successfully');
...
async function resetProducts() {
  ...
  } catch (e) { showToast('Error resetting products'); }
}

async function resetAllData() {
  if (!confirm('WARNING: This will delete ALL data from Firebase. Continue?')) return;
  try {
    var prodSnap = await db.collection('products').get();
    var batch1 = db.batch();
    prodSnap.forEach(function (doc) { batch1.delete(doc.ref); });
    await batch1.commit();
    var ordSnap = await db.collection('orders').get();
    var batch2 = db.batch();
    ordSnap.forEach(function (doc) { batch2.delete(doc.ref); });
    await batch2.commit();
    await saveConfig({ adminPass: ADMIN_DEFAULT_PASS, nextOrderId: 1001 });
    products = []; orders = []; nextOrderId = 1001;
    await adminShowSec('dashboard');
    showToast('All data has been reset');
  } catch (e) { console.error(e); showToast('Error resetting data'); }
}
...
async function showPage(page, subFilter) {
  ...
  var myVersion = ++_pageVersion;
  products = await loadProducts();
  if (myVersion !== _pageVersion) return;
...
function productCard(p) {
  ...
  html += '</div>';
  html += '<div class="p-details"><h4>' + escHtml(p.name) + '</h4>';
...
function initCategoryPage(catType, gridId, countId, filterId, subFilter) {
  ...
  _activeSizes = [];
  renderFilterSidebar(catType, filterId);
  applyFilters(catType, gridId, countId, filterId);
}

function renderFilterSidebar(catType, filterId) {
  var el = document.getElementById(filterId);
  var cats = (catType === 'accessories') ? ['watch', 'bag', 'perfume', 'jewelry', 'sunglasses', 'homedecor'] : [catType];
  var allSubs = [];
  cats.forEach(function (c) { if (SUB_CATS[c]) allSubs = allSubs.concat(SUB_CATS[c]); });
  var html = '';
  ...
  html += '</div></div>';
  html += '<div class="filter-group"><h4>Price Range</h4><div class="price-range-wrap"><input type="range" id="priceRange" min="0" max="50000" value="50000" oninput="updatePriceFilter(this.value,\'' + catType + '\')"><div class="price-range-val" id="priceRangeVal">Up to ৳50,000</div></div></div>';
...
function setSubFilter(val, catType) {
  ...
  applyFilters(catType, currentPageInfo.gridId, currentPageInfo.countId, currentPageInfo.filterId);
  renderFilterSidebar(catType, currentPageInfo.filterId);
}
...
function applyFilters(catType, gridId, countId, filterId) {
  var list = products.slice();
...
function performSearch(query) {
  ...
  _searchTimer = setTimeout(async function () {
    ...
    if (!results.length) { el.innerHTML = '<div class="search-no-result"><i class="fas fa-search"></i><br>No results found for "' + escHtml(query) + '"</div>'; return; }
...
function openPM(id) {
  ...
  document.getElementById('pmColors').innerHTML = colHtml || '<span style="color:var(--lg);font-size:13px">No colors available</span>';
  var szHtml = '';
  (p.sizes || []).forEach(function (s) {
    szHtml += '<button class="pm-sbtn' + (s === pmSelectedSize ? ' active' : '') + '" onclick="selectPMSize(this,\'' + s + '\')">' + s + '</button>';
  });
  document.getElementById('pmSizes').innerHTML = szHtml || '<span style="color:var(--lg);font-size:13px">No sizes available</span>';
...
function updateCartQty(key, dir) {
...
function renderCart() {
  ...
  cart.forEach(function (item) {
    ...
    html += '<div class="cart-item-img"><img src="' + item.image + '" onerror="this.src=\'https://via.placeholder.com/80x100/121212/d4a017?text=Img\'"></div>';
    html += '<div class="cart-item-info"><h4>' + escHtml(item.name) + '</h4>';
    ...
    html += '</div>';
    html += '<div class="cart-item-qty"><button onclick="updateCartQty(\'' + item.key + '\',-1)"><i class="fas fa-minus"></i></button><span>' + item.qty + '</span><button onclick="updateCartQty(\'' + item.key + '\',1)"><i class="fas fa-plus"></i></button></div>';
    html += '<div class="cart-item-price">' + fmtPrice(item.price * item.qty) + '</div>';
    html += '</div>';
    html += '<button class="cart-item-remove" onclick="removeFromCart(\'' + item.key + '\')"><i class="fas fa-times"></i></button>';
    html += '</div>';
  });
  el.innerHTML = html;
...
function toggleWishlist(id) {
  ...
  saveWishlist();
  updateWlBadge();
  // Re-render current grid to update heart icons
  ...
  if (grid) {
    grid.querySelectorAll('.p-card').forEach(function (card) {
      var btn = card.querySelector('.wl-btn');
      if (!btn) return;
      var onclickStr = btn.getAttribute('onclick') || '';
      var match = onclickStr.match(/toggleWishlist\((\d+)\)/);
      if (match) {
        var pid = Number(match[1]);
        var liked = wishlist.has(pid);
...
async function renderWishlist() {
  ...
  if (!products.length) products = await loadProducts();
  ...
  wlProducts.forEach(function (p) {
    html += '<div class="cart-item">';
    html += '<div class="cart-item-img" style="cursor:pointer" onclick="closeWishlist();openPM(' + p.id + ')"><img src="' + p.image + '" onerror="this.src=\'https://via.placeholder.com/80x100/121212/d4a017?text=Img\'"></div>';
    html += '<div class="cart-item-info"><h4>' + escHtml(p.name) + '</h4>';
    html += '<div class="cart-item-price" style="margin-top:4px">' + fmtPrice(p.price) + '</div>';
    html += '<div class="wl-item-actions">';
    html += '<button class="wl-action-btn wl-add-cart" onclick="addToCart(products.find(function(x){return x.id===' + p.id + '}),\'' + (p.colors[0] || '') + '\',\'' + (p.sizes[0] || '') + '\',1);closeWishlist();openCart()">Add to Cart</button>';
    html += '<button class="wl-action-btn wl-remove" onclick="toggleWishlist(' + p.id + ');renderWishlist()">Remove</button>';
    html += '</div></div></div>';
  });
  el.innerHTML = html;
}
...
function subscribeNL(e) {
  ...
  if (input.value) { showToast('Thank you for subscribing!'); input.value = ''; }
  return false;
}
